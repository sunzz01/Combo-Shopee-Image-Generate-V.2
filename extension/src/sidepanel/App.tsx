...
import { Settings as SettingsIcon, Package, Target, RotateCcw } from 'lucide-react';
...
export default function App() {
  ...
  const steps = [
    { id: 'SCAN', icon: Target, label: 'SCAN สินค้า' },
    { id: 'SELECT', icon: Package, label: 'เลือก & ส่งต่อ' }
  ];
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 bg-gradient-to-tr from-orange-500 to-amber-500 rounded-xl flex items-center justify-center shadow-lg shadow-orange-200">
            <SparklesIcon className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-base font-black tracking-tight text-slate-800 flex items-center gap-1.5">
              PicSeller Connector
              <span className="text-[10px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-md font-bold">v{extensionVersion}</span>
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={`p-2 rounded-xl transition-all ${showSettings ? 'bg-orange-100 text-orange-600' : 'text-slate-400 hover:bg-slate-100'}`}
            title="ตั้งค่าระบบ"
          >
      ...
      {!showSettings && (
        <nav className="bg-white border-b border-slate-200 px-4 py-3">
          <div className="flex items-center justify-around max-w-md mx-auto relative px-4">
            {/* Progress Line */}
            <div className="absolute top-1/2 left-1/4 right-1/4 h-0.5 bg-slate-200 -translate-y-4 z-0" />
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s.id} className="flex flex-col items-center gap-1 z-10">
                  ...
                  <span className={`text-[10px] font-extrabold uppercase tracking-tight ${isActive ? 'text-orange-600' : 'text-slate-400'}`}>
                    {s.label}
                  </span>
                </div>
              );
